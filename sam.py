...
class DronePollinationApp:
    def __init__(self, root):
        ...
        # Load YOLO model (default yolov8n.pt)
    ...
    def detect_flower_by_color(self, frame):
        ...
        if not flower_detected:
            ...
            # HSV range for pink flowers
            lower_pink = np.array([140, 50, 50])
            upper_pink = np.array([170, 255, 255])
            mask_pink = cv2.inRange(hsv, lower_pink, upper_pink)

            # HSV range for yellow flowers
            lower_yellow = np.array([20, 100, 100])
            upper_yellow = np.array([30, 255, 255])
            mask_yellow = cv2.inRange(hsv, lower_yellow, upper_yellow)

            combined_mask = cv2.bitwise_or(mask_pink, mask_yellow)
            flower_pixels = cv2.countNonZero(combined_mask)

            if flower_pixels > 500:  # threshold to tune
                flower_detected = True
    ...
    def camera_loop(self):
        ...
        while self.camera_running:
            ...
            if flower_found and not flower_already_notified:
                self.log("✅ Flower detected by YOLO/color detection!")
